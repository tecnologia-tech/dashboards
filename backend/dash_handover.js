...
// Função para obter todos os IDs das leads com status "handover" (status=2)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'handover'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 2 }, // Status 2 é "handover"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'handover' encontrados.`);
  return ids;
}
...
export default async function dashHandover() {
  ...
  console.log("▶️ Executando dash_handover...");

  try {
    // Obtém os IDs das leads com status "handover"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'handover' encontrados.`);
    ...
    console.error("🚨 Erro em dash_handover:", err.message);
  }
}

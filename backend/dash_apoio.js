...
// Função para obter todos os IDs das leads com status "apoio" (status=1)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'apoio'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 1 }, // Status 1 é "apoio"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'apoio' encontrados.`);
  return ids;
}
...
export default async function dashApoio() {
  ...
  console.log("▶️ Executando dash_apoio...");

  try {
    // Obtém os IDs das leads com status "apoio"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'apoio' encontrados.`);
    ...
    console.error("🚨 Erro em dash_apoio:", err.message);
  }
}

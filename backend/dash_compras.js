...
// Função para obter todos os IDs das leads com status "compras" (status=2)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'compras'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 2 }, // Status 2 é "compras"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'compras' encontrados.`);
  return ids;
}
...
export default async function dashCompras() {
  ...
  console.log("▶️ Executando dash_compras...");

  try {
    // Obtém os IDs das leads com status "compras"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'compras' encontrados.`);
    ...
    console.error("🚨 Erro em dash_compras:", err.message);
  }
}

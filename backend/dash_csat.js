...
async function getColumnMap() {
  ...
  const res = await fetch("https://api.monday.com/v2", {method: "POST",
    headers: {
      Authorization: MONDAY_API_KEY,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ query, variables }),}); const data = await res.json();
  const columns = data?.data?.boards?.[0]?.columns || [];

  const map = {};
  columns.forEach((col) => {
    if (col.id && col.title) {
      // 🔹 Usa título + ID para evitar duplicatas (ex: Delivery_xxx)
...
`);
    ...
    await client.end().catch(() => {});}}export default async function dashCsat() {
  const start = Date.now();
  console.log("▶️ Executando dash_csat.js...");try {
    ...
    await saveToPostgres(items, columnMap);console.log(
      `🏁 dash_csat concluído em ${((Date.now() - start) / 1000).toFixed(1)}s`
    );
  } catch (err) {
    console.error("🚨 Erro geral em dash_csat:", err.message);}
}
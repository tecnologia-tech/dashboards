...
// Função para obter todos os IDs das leads com status "csat" (status=2)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'csat'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 2 }, // Status 2 é "csat"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'csat' encontrados.`);
  return ids;
}

// Função principal do módulo, que faz a integração com o banco de dados
export default async function dashCsat() {
  const start = Date.now();
  console.log("▶️ Executando dash_csat...");

  try {
    // Obtém os IDs das leads com status "csat"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'csat' encontrados.`);
    ...
    console.log(
      `🏁 dash_csat concluído em ${((Date.now() - start) / 1000).toFixed(1)}s`
    );
  } catch (err) {
    console.error("🚨 Erro em dash_csat:", err.message);
  }
}

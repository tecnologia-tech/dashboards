...
// Função para obter todos os IDs das leads com status "fornecedor" (status=5)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'fornecedor'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 5 }, // Status 5 é "fornecedor"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'fornecedor' encontrados.`);
  return ids;
}
...
export default async function dashFornecedores() {
  ...
  console.log("▶️ Executando dash_fornecedores...");

  try {
    // Obtém os IDs das leads com status "fornecedor"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'fornecedor' encontrados.`);
    ...
    console.log(
      `🏁 dash_fornecedores concluído em ${(
        (Date.now() - start) /
        1000
      ).toFixed(1)}s`
    );
  } catch (err) {
    console.error("🚨 Erro em dash_fornecedores:", err.message);
  }
}

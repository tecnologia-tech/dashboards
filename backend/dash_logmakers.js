...
const MONDAY_BOARD_ID = "18027717616";
const TABLE_NAME = "dash_logmakers"; 
...
async function getColumnMap() {
  ...
  columns.forEach((col) => {
    if (col.id && col.title) {
      ...
      if (map[col.id]) {
        console.warn(`⚠️ Coluna duplicada: ${col.title}`);
      }
      map[col.id] = safeName;
    }
  });
...
async function saveToPostgres(items, columnMap) {
  ...
  try {
    ...
    const colDefs = uniqueColumns.map((t) => `"${t}" TEXT`).join(", ");
    await client.query(`
    ...
    const insertQuery = `
      INSERT INTO ${TABLE_NAME} (id, name, grupo, ${uniqueColumns
      .map((c) => `"${c}"`)
      .join(", ")})
      VALUES (${[
        "$1",
...
export default async function dashCompras() {
  ...
  console.log("▶️ Executando dash_logmakers.js...");
  try {
    ...
    console.log(
      `🏁 dash_logmakers concluído em ${((Date.now() - start) / 1000).toFixed(
        1
      )}s`
    );
  } catch (err) {
    console.error("🚨 Erro geral em dash_logmakers:", err.message);
  }
}

...
// Função para obter todos os IDs das leads com status "delivery" (status=4)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'delivery'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 4 }, // Status 4 é "delivery"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'delivery' encontrados.`);
  return ids;
}
...
export default async function dashDelivery() {
  ...
  console.log("▶️ Executando dash_delivery...");

  try {
    // Obtém os IDs das leads com status "delivery"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'delivery' encontrados.`);
    ...
    console.log(
      `🏁 dash_delivery concluído em ${((Date.now() - start) / 1000).toFixed(
        1
      )}s`
    );
  } catch (err) {
    console.error("🚨 Erro em dash_delivery:", err.message);
  }
}

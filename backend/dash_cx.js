...
// Função para obter todos os IDs das leads com status "cx" (status=5)
async function getAllLeadIds() {
  const ids = [];
  console.log("🧭 Iniciando a busca de leads 'cx'...");

  for (let page = 1; ; page++) {
    console.log(`📄 Buscando leads na página ${page}...`);
    const leads = await callRPC("findLeads", {
      query: { status: 5 }, // Status 5 é "cx"
      page,
      limit: 100,
    });
  ...
  console.log(`📦 Total de ${ids.length} leads 'cx' encontrados.`);
  return ids;
}

// Função principal do módulo, que faz a integração com o banco de dados
export default async function dashCx() {
  const start = Date.now();
  console.log("▶️ Executando dash_cx...");

  try {
    // Obtém os IDs das leads com status "cx"
    const leadIds = await getAllLeadIds();
    console.log(`📦 ${leadIds.length} leads 'cx' encontrados.`);
    ...
    console.log(
      `🏁 dash_cx concluído em ${((Date.now() - start) / 1000).toFixed(1)}s`
    );
  } catch (err) {
    console.error("🚨 Erro em dash_cx:", err.message);
  }
}
